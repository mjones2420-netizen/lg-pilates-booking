...
Last updated: 23 Aug 2026 (session 90 — #28 closed, #29 T1-09c Stripe refund sync shipped)
...
- **Manual step still needed from Mark, both Stripe accounts**: add `charge.refunded` to the subscribed events list on the webhook endpoint in the Stripe dashboard (test + prod) — the webhook code silently ignores event types it isn't subscribed to receive, so this won't do anything live until that's ticked on both.
