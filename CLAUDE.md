...
Last updated: 24 Aug 2026 (session 93 — waitlist backend #72 + #73 built, TEST only)
...
**Session 93 (2026-08-24):** Waitlist backend built — [#72](https://github.com/mjones2420-netizen/lg-pilates-booking/issues/72) (migration) + [#73](https://github.com/mjones2420-netizen/lg-pilates-booking/issues/73) (edge functions). Commits `36a7bfa` + `d3dec81`. **Applied/deployed to TEST ONLY — production deliberately untouched.** 276/276 local, CI green.

- **⚠️ HARD CONSTRAINT FOR THE NEXT SESSION: migration 28 must NOT reach production until [#74](https://github.com/mjones2420-netizen/lg-pilates-booking/issues/74) (public site) ships.** The DB now applies the reservation rule (`booked + wait >= cap` → CLASS_FULL), but `index.html:1733` still computes `spacesLeft = blk.cap - blk.booked`. The moment any waitlist row exists on a prod block, the page advertises a space the DB refuses — and in Stripe mode `stripe-checkout` does no capacity check, so the customer is **charged first** and only the webhook discovers CLASS_FULL. Caught by code review, not by tests.
- **Migration is 28, not 25.** #72's title was written in session 63 before 25/26/27 were taken; title updated on the issue. File `tests-playwright/migrations/28_waitlist.sql`.
- **#72 shipped**: `waitlist` reshaped from class-scoped scaffolding to block-scoped (was empty on BOTH projects — verified first, so no data migration). Adds `block_id` (FK CASCADE), `status` waiting/offered, `offer_token` uuid (partial unique index), `offered_at`, `joined_alert_sent_at`, `offer_email_sent_at`, UNIQUE(block_id, customer_id), customer FK recreated CASCADE. New `sync_block_wait_count()` trigger keeps `blocks.wait` current (**display only** — every gate uses a live COUNT under the block row lock). New RPCs: `join_waitlist` (anon), `offer_waitlist_space` / `release_waitlist_hold` (admin, `is_admin()` + anon REVOKEd), `get_offer_details` (anon, token-gated prefill). `pending_bookings.offer_token` added for the Stripe round trip.
- **`book_if_available` v3** — DROP+CREATE (new param = new signature, never overload). No token: reservation rule. With token: matched on token+block+`status='offered'` (WL_BAD_TOKEN) AND `customer_id` (WL_TOKEN_MISMATCH), then physical cap only; deletes the hold **after** the booking insert succeeds. **DROP destroys the ACL** — anon EXECUTE re-granted explicitly. Verified by live REST call that BOTH the legacy 4-arg form (≈30 spec files) and the 5-arg form still resolve.
- **`admin_delete_class` repaired** in the same migration — migration 21's body ran `DELETE FROM waitlist WHERE class_id`, a column this migration drops. Now a `block_id IN (SELECT ...)` subquery. Deletes stay explicit/ordered (session 66 FK-ordering lesson).
- **#73 shipped**: `send-email` gains `waitlist_joined_alert` (PUBLIC — recipient is `settings.admin_email` only, one-shot via `joined_alert_sent_at`) and `waitlist_offer` (**TRUSTED, not public** — deliberate deviation from the issue's wording: it carries a working booking token and is only ever sent from the admin dashboard right after the admin-gated offer; mirrors the existing new_booking_alert-public / card_payment_alert-trusted split). New `loadWaitlistContext()`. `buildOfferLink()` validates any caller `app_url` against ALLOWED_ORIGINS and discards caller query/hash — never embeds an unvalidated URL in an email from our domain. `stripe-checkout` validates the hold (uuid shape → block+status → email match) BEFORE contacting Stripe so nobody is charged for a doomed booking. `stripe-webhook` forwards `p_offer_token` and maps WL_BAD_TOKEN / WL_TOKEN_MISMATCH. Deployed TEST: send-email v15, stripe-checkout v16, stripe-webhook v13 (`verify_jwt:false` preserved).
- **Code review: 7 findings, 6 fixed pre-push.** Two were real bugs of mine: (1) `join_waitlist` passed `'returning'` to `upsert_customer`, silently relabelling existing customers — a `new` client would lose their PAR-Q prompt; now passes the EXISTING `customer_type` straight back. (2) caller-supplied `isTest:true` on the anon path would claim the one-shot stamp and skip Resend on PROD, so Louise silently never hears about a booking — now gated on `TEST_BYPASS_ENABLED` (#35 pattern), which **also closes the same pre-existing hole on `reserved_confirmation`/`new_booking_alert` from session 71**. Confirmed via `supabase secrets list` that TEST has the secret and PROD does not. Also fixed: ended/hidden blocks were joinable; client payment-failed email hardcoded "class filled up" (untrue for the WL reasons, shown next to a card charge).
- **Added beyond the checklist, deliberately**: `WL_NO_SPACE` on `offer_waitlist_space` (`booked + offered >= cap`) so two holds can't be minted against one seat, and `WL_LIST_FULL` capping the list at the block's cap. Same reasoning as #59 — the DB is the gate, not the UI.
- **Security review: no findings.** Cleared open-relay, offer-link origin validation, esc() coverage, token binding, `get_offer_details` PII, grants, `search_path`, and the stripe-checkout bypass.
- **[#106](https://github.com/mjones2420-netizen/lg-pilates-booking/issues/106) filed + DEFERRED by Mark** (sub-issue of #71, bottom of Todo): anon `join_waitlist` has no per-IP throttle. `WL_LIST_FULL` bounds the damage but does not close it — unbounded fake joins would lock a class out of public sale (reservation rule) and burn the Resend quota one alert per row. Fix is the #35 shape: a throttled edge function **plus revoking the direct anon EXECUTE** (the half that actually closes it). Blocker for #71 going live, not for building #74/#75/#76.
- **CU-08 CI flake root-caused and fixed** (`d3dec81`, test-only). Failed 3/3 on CI, passed locally. The class picker is built synchronously inside `openRecordSwapModal()` from the `blocks` global and **never rebuilds**; if the login fetch hasn't landed the picker holds only its placeholder → "element(s) not found". #79 added a wait for the *swaps* fetch (date picker) and left this one unguarded. Now waits for the block to be present before opening the modal. Note `blocks` starts as `[]`, so presence of the row is the only meaningful ready signal.
- **No new specs this session** — WL-01..13 belong to [#76](https://github.com/mjones2420-netizen/lg-pilates-booking/issues/76) by design. The new waitlist paths are proven only by a 15-step manual SQL lifecycle smoke test (incl. the reservation rule: seat free at booked=1/cap=2 but 2 waiting → public booking still refused). Test count unchanged at **276**; TEST-PLAN.md not regenerated (no test added/removed).
- **[#30](https://github.com/mjones2420-netizen/lg-pilates-booking/issues/30) folded into the release plan** — was a standalone issue despite RELEASE-PLAN.md referencing it. Now a native sub-issue of [#68](https://github.com/mjones2420-netizen/lg-pilates-booking/issues/68) (Phase 3) and repositioned on the board next to it. Left OPEN — the key swap itself hasn't happened.
- **New global convention (all projects)**: `~/.claude/CLAUDE.md` gained a "Milestone recap" section — an unprompted plain-English summary (what it means / what's live / actions needed / mandatory honest caveats) whenever work ships or a decision is waiting; the one exception to the bullets rule. Manual triggers: `/summary` (new global command), "plain english summary", "full recap", "catch me up". **`/recap` is Claude Code's own built-in short version — different thing.**
